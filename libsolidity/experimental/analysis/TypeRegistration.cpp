...
bool TypeRegistration::visit(BuiltinDefinition const& _builtinDefinition)
{
	if (annotation(_builtinDefinition).typeConstructor)
		return false;
	...
	auto primitiveType = [&](std::string _name) -> std::optional<PrimitiveType> {
		...
		return std::nullopt;
	}(_builtinDefinition.nameParameter());

	if (!primitiveType.has_value())
		m_errorReporter.fatalTypeError(
			7758_error,
			_builtinDefinition.location(),
			"Expected the name of a built-in primitive type."
		);

	annotation(_builtinDefinition).typeConstructor = m_typeSystem.constructor(primitiveType.value());
	return true;
}
...
void TypeRegistration::endVisit(TypeDefinition const& _typeDefinition)
{
	...
	if (auto const* builtinDefinition = dynamic_cast<BuiltinDefinition const*>(_typeDefinition.typeExpression()))
	{
		auto [previousDefinitionIt, inserted] = annotation().builtinTypeDefinitions.try_emplace(
			builtinDefinition->nameParameter(),
			&_typeDefinition
		);

		if (inserted)
			annotation(_typeDefinition).typeConstructor = annotation(*builtinDefinition).typeConstructor;
		else
		{
